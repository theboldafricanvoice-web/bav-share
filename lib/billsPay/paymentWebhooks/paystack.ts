...
export const paystackBillsPayPaymentWebhookAdapter: BillsPayPaymentWebhookAdapter = {
  ...
  async verifyAndNormalize(
    ...
    if (!secretKey) {
      throw new Error("PAYSTACK_SECRET_KEY is missing for bills-pay webhook verification.");
    }
    ...
    let normalizedEventType: NormalizedBillsPayPaymentWebhookEvent["eventType"] | null =
      null;
